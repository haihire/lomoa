  ...
  async upsertUsers(rows: UserUpsertRow[]): Promise<void> {
    for (const row of rows) {
      await this.prisma.$executeRaw`
        ...
        VALUES (
          ${row.server}, ${row.name}, ${row.level}, ${row.combatPower}, ${row.classIdx},
          ${row.thesix}, ${row.expeditionKey}, ${row.coreSun}, ${row.coreMoon}, ${row.coreStar},
          ${row.statCrit}, ${row.statSpec}, ${row.statSwift}, ${row.statBuild}
        )
